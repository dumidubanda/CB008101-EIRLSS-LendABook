...
public class VideosController
{
    ...
    @PostMapping
    public Videos create(@Valid @RequestBody Videos video) { return videosService.save(video); }

    @PutMapping
    public Videos update(@Valid @RequestBody Videos video){ return videosService.save(video); }

    @DeleteMapping(path = "video/{id}")
    public void delete(@PathVariable("id") long id){ videosService.deleteById(id); }
    ...
    @GetMapping
    public List<Videos> list() { return videosService.findAll(); }

    @GetMapping(path = "search/{name}")
    public List<Videos> search(@PathVariable("name") String name){ return videosService.findAllByName(name); }
}
